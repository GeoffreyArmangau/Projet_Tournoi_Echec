class MatchesController:
    def __init__(self):
        """
        Initialise le contrôleur des matches pour gérer les opérations liées aux matchs d'un tournoi.
        """
        pass

    def set_score(self, score_1, score_2):
        """
        Attribue et valide les scores d'un match.
        Lève une exception si les scores ne sont pas valides ou si la somme n'est pas 1.
        """
        ...
        if score_1 not in valid_scores or score_2 not in valid_scores:
            raise ValueError(
                "Score invalide: les scores doivent être 0, 0.5 ou 1")

        # Vérifie que les scores de tous le match soient entrés
        if score_1 + score_2 != 1:
            raise ValueError(
                "le score total du match doit être de 1 pour couvrir "
                "un gagnant + un perdant ou une égalité")

        return True

    def get_winner(self, match):
        """
        Retourne une chaîne indiquant le gagnant du match ou l'égalité.
        """
    ...
    def is_completed(self, match):
        """
        Vérifie si un match est terminé (au moins un score non nul).
        """
        return match.score1 != 0 or match.score2 != 0

    def match_to_dictionary(self, match):
        """
        Retourne les informations du match sous forme de dictionnaire sérialisable.
        """
        return {
    ...
    def get_match_status(self, match):
        """
        Retourne une chaîne représentant le statut du match (terminé ou en attente).
        """
        if self.is_completed(match):
            return (f"{match.player1.first_name} {match.score1} : "
                    f"{match.player2.first_name} {match.score2} / Match terminé")
        else:
            return "En attente"
